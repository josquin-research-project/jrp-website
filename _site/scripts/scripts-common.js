...
var BASEADDR       = "josquin.stanford.edu" ; // Base address of URL.
...
console.log("GOT HERE AAA");
      localStorage.WORKLIST = ReadFile('http://josquin.stanford.edu/includes/worklist.json');
      if (localStorage.WORKLIST.match(/^\s*$/)) {
console.log("GOT HERE BBB");
         localStorage.WORKLIST = ReadFile('http://josquin.stanford.edu/data?a=worklist-json'); 
      }
...
function DisplayCriticalNotes(jrpid, target) {
   ReadFileAsync("http://josquin.stanford.edu/data?id=" + jrpid + "&a=critical", function(responseText) {
